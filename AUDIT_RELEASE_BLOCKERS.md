...
| S0 | 4 | **4** | 0 |
| S1 | 11 | **11** | 0 |
| S2 | 9 | **7** | 2 (S2-4 stale `window_geometry` property, S2-6 unused `download_async`, S2-9 tag-restore UI — deferred to v1.1) |
| S3 | 5 | 1 | 4 (deferred to v1.1) |
| Commercial-release C-deltas | 11 | **11** | 0 |

**All S0 and S1 blockers are fixed.** Audit fixes landed in commits `6bc1097` through `ee89e18`. Commercial-release prep (version unification, CLI release flags, startup preflight, PyInstaller, Inno Setup, bundled FFmpeg, Playwright graceful degradation, third-party notices, release checklist, packaging CI, final wording sweep) landed in `65e2938` through `<this commit>` and is fully accounted for in [Commercial-release deltas](#commercial-release-deltas) below.
...
Tracking the extra release-prep work required for a true commercial Windows EXE distribution. These are not "bugs" the audit found — they are the gap between "branch passes tests" and "ship a signed installer to paying users." Each landed in its own commit on `audit/full-line-audit`.

| Delta | Severity | Status |
|---|---|---|
| **C-1**: Single source of truth for the app version (`version.py` module read by `update_checker`, `cli --version`, `main.py` Qt metadata, packaging, and EXE VS_VERSIONINFO) | S1 for release | **fixed** in commit `65e2938`. Evidence: `version.py` owns `__version__`, `PRODUCT_NAME`, `COMPANY_NAME`, `COPYRIGHT`; `core/update_checker.py:CURRENT_VERSION` imports from it; `main.py` sets Qt metadata from it; `packaging/generate_version_info.py` reads it for VS_VERSIONINFO; `packaging/ytspot.iss` reads ProductVersion from the built EXE itself. `tests/test_p0_gates.py::TestVersionConsistency` (3 tests) guards drift between `version.py`, `update_checker`, and `pyproject.toml`. |
| **C-2**: CLI `--version` and `--doctor` flags | S1 for release | **fixed** in commit `55ec627`. Evidence: `cli.py` makes `url` optional (`nargs="?"`); `--version` uses argparse native action; `--doctor` calls `_run_doctor` which prints the 5-check preflight detail and exits 0/1. `tests/test_p0_gates.py::TestCLIReleaseFlags` (3 tests). |
| **C-3**: Wire `error_handler.run_preflight()` into `main.py` startup and extend it with cookies + output-dir + Playwright checks | S1 for release | **fixed** in commit `55ec627`. Evidence: `main.py` step 8 calls `run_preflight(output_dir, cookies_file)` after `window.show()` and surfaces `warning_text()` via `qfluentwidgets.MessageBox` if blocking checks fail; `error_handler.run_preflight` extended with `check_output_dir_writable`, `check_cookies_file_valid`, `check_playwright`; reports per-check detail for the CLI doctor. |
| **C-4**: PyInstaller spec, build script, Windows VS_VERSIONINFO, application icon | S0 for release | **fixed** in commit `040875c`. Evidence: `packaging/ytspot.spec` (one-folder build, GUI + CLI EXEs, hidden imports for qfluentwidgets/ytmusicapi/yt-dlp); `packaging/generate_version_info.py` (VS_VERSIONINFO generator); `packaging/generate_icon.py` + `packaging/ytspot.ico` (multi-resolution ICO); `scripts/build_windows.ps1` (full orchestrator with tests, build, ZIP, SHA-256). |
| **C-5**: Inno Setup installer config | S1 for release | **fixed** in commit `040875c`. Evidence: `packaging/ytspot.iss` (English + Hebrew, per-user install allowed, optional Playwright install task, Start Menu / Desktop shortcuts, reads ProductVersion from EXE). |
| **C-6**: Bundle LGPL FFmpeg/ffprobe + auto-discovery in downloader | S0 for release | **fixed** in commit `42fac08`. Evidence: `utils/paths.py:get_bundled_ffmpeg_dir`/`get_ffmpeg_executable` discover binaries next to `ytspot.exe` (frozen) or under `packaging/ffmpeg/` (source); `core/downloader.py:_build_ydl_opts` passes `ffmpeg_location` to yt-dlp; `core/hls_downloader.py:_find_ffmpeg/_find_ffprobe` delegate to `utils.paths`; `error_handler.check_ffmpeg` reports the bundled binary correctly. Build script + THIRD_PARTY_NOTICES.md make clear only the **LGPL** build may be bundled. |
| **C-7**: Playwright graceful degradation + clear "install browsers" path | S1 for release | **fixed** in commit `f515e78`. Evidence: `utils/playwright_check.py` (single helper with `is_playwright_available`, `require_playwright_or_raise`, `PlaywrightNotAvailable` exception carrying English + Hebrew messages); all 5 Playwright-using modules in `core/` (scraper, cookie_wizard, channel_tab_discoverer, listing_scraper, universal_extractor) lifted module-level imports into deferred ones and pre-check; `ui/app_window.py:_run_cookie_wizard_ui` pre-checks and shows `MessageBox(message_he)`; `scripts/install_playwright.ps1` is the post-install path. `tests/test_p0_gates.py::TestPlaywrightDegradation` (3 tests). |
| **C-8**: `THIRD_PARTY_NOTICES.md` covering all bundled and runtime deps | S0 for release (legal) | **fixed** in commit `fd0ac46`. Evidence: enumerates every runtime dep + bundled FFmpeg; explicitly flags **Mutagen (GPL v2+)**, **PySide6-Fluent-Widgets (GPL v3 / Commercial)**, **PySideSix-Frameless-Window (GPL v3)** as REVIEW REQUIRED with options (acquire commercial license, open-source the app, or replace the dependency); pre-release checklist for the maintainer to walk through with counsel. |
| **C-9**: `RELEASE_CHECKLIST.md` for manual smoke on a clean Windows machine | S1 for release | **fixed** in commit `fd0ac46`. Evidence: full manual checklist covering pre-release, build commands, cold launch on a Python-less VM, functional smoke (15 user flows), error/edge cases (missing FFmpeg, offline, unwritable output, invalid cookies, sign-in wizard without Playwright), Playwright install verification, installer smoke, antivirus/SmartScreen review, pre- and post-publish steps. |
| **C-10**: Windows packaging CI workflow with SHA-256 artifact checksums (no auto-publish) | S2 for release | **fixed** in commit `ba0cf72`. Evidence: `.github/workflows/release-windows.yml` runs on tag push `v*.*.*` or manual dispatch (NOT every PR); runs full pytest, PyInstaller, EXE smoke (`--version` + `--doctor`); zips + SHA-256s; uploads portable folder + ZIP+checksums as two artifacts. Does NOT auto-publish to GitHub Releases — maintainer attaches manually after the manual smoke passes. |
| **C-11**: Final grep sweep for bypass/anti-ban/circumvent/DRM in user-facing strings and public docs | S1 for release | **fixed** in this commit. Evidence: repo-wide `grep -i "bypass\|anti.ban\|circumvent\|DRM\|לעקוף\|עקוף"` returns only (a) audit/release docs describing the cleanup, (b) internal class name `BotBypassWindow` + tempfile names `ytspot_bypass_*.txt` (S2-8 / S3-2 deferred — not user-visible), (c) accurate technical descriptions in tooltips (`for age-gated or private content`), (d) PowerShell `-ExecutionPolicy Bypass` syntax in build scripts, (e) statements in README/THIRD_PARTY_NOTICES that we DO NOT bypass DRM. No user-facing copy uses the risky vocabulary. |
